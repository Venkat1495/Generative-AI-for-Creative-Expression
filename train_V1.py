...
def generate(device, tokenizer, model, input, print_msg, max_length: int = 100):
    ...
    with torch.no_grad():
        ...
        encoded_input = tokenizer.encode_ordinary(input)
        input_ids = torch.tensor(encoded_input, dtype=torch.long).to(
            device)
        ...
        for _ in range(max_length - len(encoded_input)):
            ...
            print(f"Next Word: {tokenizer.decode([idx_next])}", end=' ')
            # Break if [EOS]
...
def train_model(config):
    ...
    for epoch in range(initial_epoch, config['num_epochs']):
        ...
        for j, batch in enumerate(batch_iterator):
            ...
            if j % 2000 == 0  and j != 0:  # and average_val_loss < best_val_loss
        ...
        # input = torch.zeros((1, 1), dtype=torch.long, device=device)
        print(f"Input After Encode and coverted to tensor: {input}")
        y = model.generate(config, input, 250)
        print(f"\n\nGenerated Output:\n{tokenizer_src.decode(y[0].tolist())}")
        # generate(device, tokenizer_src, model, config['generate_input'], lambda msg: batch_iterator.write(msg), 50)
        model.train()
        # generate(device, tokenizer_src, model, config['generate_input'], lambda msg: batch_iterator.write(msg), 50)
